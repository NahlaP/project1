...
// /* ---------------- explicit override models (legacy sir-template) ---------------- */
...
// /**
//  * Dashboard expects certain (canonical) section type names for preview components.
//  * Normalize incoming defaults so they render:
//  *   works              -> projects
//  *   clients            -> brands
//  *   servicesAccordion  -> services
//  *   blogList           -> blog
//  * Also fold common singular/plural/alias cases for gym-template:
...
//   // Optional default titles when the source used generic names
...
//   return HIDE_WORKS
//     ? list.filter((r) => canonicalType(r.type) !== "projects") // hide former "works"
//     : list;
// }
...
//   if (!SectionModel) {
//     return { ok: false as const, reason: "Section model not found" };
//   }
...
//     // which version? e.g. ?ver=v1
//     const verTag = String((req.query?.ver ?? "") as string).trim() || undefined;

//     // ---- SECTION KEYS (explicit models + gym-only layout types) ----
//     // If model is null, it's a "layout-only" type (typical for gym-template).
...
//       { key: "blog",         label: "Blog",           model: Blog },      // normalized from blogList
//       { key: "contact",      label: "Contact",        model: Contact },
//       { key: "footer",       label: "Footer",         model: Footer },

//       // gym-template types (layout-only):
//       { key: "features",     label: "Features",       model: null },
//       { key: "whychooseus",  label: "Why Choose Us",  model: null },
//       { key: "appointment",  label: "Appointment",    model: null },
//       { key: "team",         label: "Team",           model: null },
//       { key: "testimonials", label: "Testimonials",   model: null },

//       { key: "layout",       label: "Page Layout",    model: null }, // virtual
//     ];
...
//         onlyParam
//           .split(",")
//           .map((s) => canonicalType(s)) // accept aliases like testimonial→testimonials
//           .filter(Boolean)
//       );
...
//     // 1) Wipe data overrides for explicit models (sir-template style)
...
//     // Decide if we should reseed layout.
//     // If caller requested layout-only types (e.g., ?only=team,testimonials) we still reseed.
...
//     // 2) Reseed layout from version defaults (content + order)
...
//     // Normalize list → canonical types, keep order, and optionally FILTER by ?only=...
//     const allowSet =
//       onlyParam
//         ? new Set(
//             onlyParam
...
//       ...(allowSet
//         ? { type: { $in: Array.from(allowSet.values()) } } // partial reset (only selected types)
//         : {}),
//     });
...
//   const t = canonicalType(out.type);
//   out.type = t;
//   if (t === "projects" && (!out.title || /^works$/i.test(String(out.title)))) out.title = "Projects";
//   if (t === "brands" && (!out.title || /^clients$/i.test(String(out.title)))) out.title = "Brands";
//   if (t === "services" && (!out.title || /services.*accordion/i.test(String(out.title || "")))) out.title = "Services";
//   if (t === "blog" && (!out.title || /^blog\s*list$/i.test(String(out.title || "")))) out.title = "Blog";
//   return out;
// }
// function normalizeListForDashboard(rows: any[]) {
//   const list = Array.isArray(rows) ? rows.map(normalizeTypeForDashboard) : [];
...
//     // 1) Wipe user overrides for explicit models
//     const dataTargets = targets.filter((t) => t.model);
//     if (dataTargets.length) {
//       await Promise.allSettled(
//         dataTargets.map((t) => t.model!.deleteMany({ userId, templateId }))
...
//     if (!SectionModel) {
//       return res.status(400).json({ ok: false, error: "Section model not found; cannot reseed layout." });
//     }

//     // 2) Reseed layout from TemplateV
//     const home = await ensureHomePageId(userId, templateId);
//     if (!home.ok || !home.id) {
//       return res.status(400).json({ ok: false, error: home.reason || "Could not resolve Home page id" });
//     }
...
//     const defaultsRaw = pickVersionDefaults(tpl, verTag);
//     if (!defaultsRaw.length) {
//       return res.status(404).json({ ok: false, error: "No defaultSections found for this template/version." });
//     }
...
//       templateId,
//       parentPageId: home.id,
//       ...(allowSet ? { type: { $in: Array.from(allowSet.values()) } } : {}),
//     });
...
//     return res.json({
//       ok: true,
//       message: "✅ Template reset complete. Pulled content + section order from version defaults.",
//       userId,
//       templateId,
//       layout: {
//         ok: true,
