...
const { COOKIE_NAME, COOKIE_SECURE, STRIPE_SECRET_KEY } = process.env as Env;
const cookieName = COOKIE_NAME || "auth_token";
const cookieSecure = COOKIE_SECURE === "true";

// 🔹 Stripe client (for syncing email)
if (!STRIPE_SECRET_KEY) {
  console.warn(
    "[changeauth] STRIPE_SECRET_KEY missing – changeEmail will NOT sync to Stripe."
  );
}
 ...
 * POST /api/auth/change-email
 * body: { currentPassword, newEmail }
 * Also syncs email to Stripe customer (if stripeCustomerId exists)
 * --------------------------------------------------- */
export async function changeEmail(req: AuthedReq, res: Response) {
  try {
    const userId = req.user?.userId;
    ...
    // 🔹 First, sync with Stripe (so if this fails, DB/email won't change)
    if (stripe && (user as any).stripeCustomerId) {
      try {
        ...
        console.error("[changeauth.changeEmail] Stripe sync failed", err);
        return res.status(502).json({
          error:
            "Failed to update email in billing system. Please try again in a moment.",
        });
      }
    }

    // 🔹 Now update local user email
    user.email = newEmail;
    await user.save();
