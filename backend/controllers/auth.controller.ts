...
export async function signup(req: Request, res: Response) {
  try {
    ...
    const company = String(req.body?.company || "").trim() || null;
    const country = String(req.body?.country || "").trim() || null;
    const mobile = String(req.body?.mobile || "").trim() || null; // ✅ NEW
    const email = String(req.body?.email || "").trim().toLowerCase();
    const password = String(req.body?.password || "");
    ...
    // (optional) if you want to FORCE country + mobile:
    // if (!country || !mobile) {
    //   return res
    //     .status(400)
    //     .json({ error: "country and mobile are required" });
    // }
    ...
    try {
      ...
      // ignore Stripe customer creation errors here
    }
    ...
    if (stripe && stripeSubId) {
      try {
        ...
        try {
          ...
          console.error("[auth.me] Failed to sync user subscription:", err);
        }
      } catch (err) {
        console.error("[auth.me] Failed to fetch Stripe subscription:", err);
      }
    }
