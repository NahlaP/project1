...
export async function getStorageSummary(req: AuthedReq, res: Response) {
  try {
    ...
    // 2) In parallel: S3 usage + Stripe allowance + EC2 disk
    const [s3Bytes, allowance, ec2DiskRaw] = await Promise.all([
      getS3UsageBytes().catch((err) => {
        console.error("[storage] error while listing S3 objects", err);
        return 0;
      }),
    ...
    if (ec2DiskRaw) {
      const ec2Disk: any = ec2DiskRaw as any; // TS: shut up, we know it has bytes fields

      const totalBytes =
        typeof ec2Disk.totalBytes === "number" ? ec2Disk.totalBytes : null;
      const usedBytesEc2 =
        typeof ec2Disk.usedBytes === "number" ? ec2Disk.usedBytes : null;

      ec2TotalGb =
        totalBytes != null ? totalBytes / (1024 * 1024 * 1024) : null;
      ec2UsedGb =
        usedBytesEc2 != null ? usedBytesEc2 / (1024 * 1024 * 1024) : null;

      ec2UsedPercent =
        ec2TotalGb != null &&
        ec2UsedGb != null &&
        ec2TotalGb > 0
          ? (ec2UsedGb / ec2TotalGb) * 100
          : null;
    }
