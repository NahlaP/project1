...
// ✅ use auto-recording middlewares (keep upload too)
import { upload, uploadImage, uploadVideo, uploadPoster } from "../middleware/upload";
...
router.get("/", async (req: any, res, next) => {
  try {
    req.params = req.params || {};
    req.params.userId =
      req.query.userId || req.params.userId || "demo-user";
    req.params.templateId =
      req.query.templateId || req.params.templateId || "gym-template-1";
    return (await import("../controllers/hero.controller")).getHero(req, res);
  } catch (e) {
    next(e);
  }
...
const heroPosterUpload = (req: any, res: any, next: any) => {
  ...
  // AFTER: same upload + auto Media record
  return uploadPoster(req, res, next);
};

const upsertHandler =
  (hero as any).upsertHero ||
  (hero as any).updateHero ||
  (hero as any).saveHero;
...
router.post(
  "/:userId/:templateId/video",
  heroVideoUpload,
  hero.uploadHeroVideo
);
router.post(
  "/:userId/:templateId/poster",
  heroPosterUpload,
  hero.uploadHeroPoster
);
