...
export default function SignIn() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      // Read ?next=/email-manager from URL (set by middleware)
      ...
      // Default target from backend
      let target = map[me?.next] || "/choose-plan";

      // If backend says dashboard and we have a valid "next" → go there instead
      if (me?.next === "dashboard" && nextParam) {
        target = nextParam;
      }

      // Final redirect (relative URL, no localhost hard-coding)
