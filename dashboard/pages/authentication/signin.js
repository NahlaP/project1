...
export default function SignIn() {
  ...
  async function onSubmit(e) {
    ...
    // 🔹 make sure any old token is cleared first
    clearToken();

    try {
      // 1) Login → api.login THROWS if email/password are wrong
      const res = await api.login(email.trim().toLowerCase(), password);
      if (!res?.token) throw new Error(res?.error || "No token returned");
      setToken(res.token);
      ...
      setMsg({
        type: "ok",
        text: "Signed in. Checking your subscription…",
      });
      const me = await api.me(); // -> { next: 'dashboard' | 'checkout' | 'choose-plan' }
      ...
      const target = map[me?.next] || "/choose-plan";
