...
export default function SignUp() {
  ...
  const isStrong = hasMinLength && hasUpper && hasNumber;

  function validPassword(pw) {
    return pw.length >= 8 && /[A-Z]/.test(pw) && /\d/.test(pw);
  }

  // Simple mobile validation (you can make stricter later)
  function validMobile(num) {
    const n = num.replace(/[^\d]/g, ""); // remove spaces, +, -
    return n.length >= 7; // just check length; backend can validate more
  }
  ...
  async function onSubmit(e) {
    ...
    if (!validPassword(password)) {
      return setMsg({
        type: "error",
        text:
          "Password must be 8+ chars and include an uppercase letter and a number.",
      });
    }
    if (!country) {
      return setMsg({ type: "error", text: "Please select your country of origin." });
    }
    ...
    setLoading(true);
    try {
      // ✅ send mobile & country to backend
      await api.signup(fullName, company, country, mobile, email, password);

      setMsg({
        type: "ok",
        text:
          "Account created. Please sign in to continue to plan & domain setup…",
      });
      router.replace("/authentication/signin");
    } catch (e) {
      setMsg({ type: "error", text: e?.message || "Signup failed" });
    ...
    <>
        ...
        >
          ...
          <div className="row-2">
            <div className="col">
              ...
              {/* You can later replace this with a plugin (react-select, country-list, etc.) */}
              <select
          ...
          <div className={`strength strength-${strengthClass}`}>
            <div className="strength-bar">
              <span className={`dot ${password ? "on" : ""}`} />
              <span className={`dot ${hasMinLength ? "on" : ""}`} />
              <span className={`dot ${isStrong ? "on" : ""}`} />
            </div>
          ...
          <div className="foot">
            ...
            <a href="/authentication/signin" className="link">
              Sign in
            </a>
          </div>
        </form>
      ...
      <style jsx>{`
        ...
        .btn {
          ...
          cursor: pointer;
          box-shadow: 0 16px 40px rgba(255, 59, 48, 0.35);
        }
        ...
        .strength-weak .dot.on {
          background: #f97316;
        }
        .strength-medium .dot.on {
          background: #facc15;
        }
        .strength-strong .dot.on {
          background: #22c55e;
        }
        .strength-label {
          min-width: 50px;
