...
const SettingsPage = () => {
  ...
  // Change email + auto logout + redirect to login
  const handleChangeEmail = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      // 1) Call backend to change email
      await api.changeEmail(emailCurrentPassword, newEmail);

      // 2) Update local state (not very important since we redirect)
      setEmail(newEmail);
      setNewEmail("");
      setConfirmNewEmail("");
      setEmailCurrentPassword("");
      ...
      // 4) Auto-logout + redirect to login page
      setTimeout(async () => {
        try {
          if (api.logout) {
            await api.logout(); // clears cookie/session if implemented
          }
        } catch (logoutErr) {
          console.error("Logout after email change failed", logoutErr);
        } finally {
  ...
  // Change password + auto logout + redirect to login
  const handleChangePassword = async (e) => {
    e.preventDefault();
