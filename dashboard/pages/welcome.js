...
// // robust token getter (cookie -> localStorage)
// function getToken() {
//   if (typeof document !== "undefined") {
//     const m = document.cookie.match(/(?:^|;\s*)auth_token=([^;]+)/); // match your cookie name
...
//     const t = getToken();
//     const r = await fetch(url, { headers: { Authorization: t ? `Bearer ${t}` : "" } });
//     if (!r.ok) {
...
//   // Display helpers (no DB side effects)
...
//   // 🔐 NEW: call backend to verify + sync user; routes to next screen
...
//     const r = await fetch(`${BACKEND}/api/billing/verify`, {
//       method: "POST",
//       headers: {
...
//     // j: { ok, status, next, priceId? }
//     if (j.next === "dashboard") {
//       router.replace("/dashboard");
//       return;
//     }
//     if (j.next === "checkout") {
//       const q = j.priceId ? `?priceId=${encodeURIComponent(j.priceId)}` : "";
//       router.replace(`/checkout${q}`);
//       return;
//     }
//     router.replace("/choose-plan");
//   }, [sessionId, subId, router, BACKEND]);
...
//       // After showing summary, attempt verification → route accordingly
//       await verifyBilling();
//     } catch {
//       setUi((x) => ({
//         ...x,
