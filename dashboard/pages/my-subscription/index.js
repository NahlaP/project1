  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // ⚠️ REMOVED: old priceId-based guess (it was always 109 or 199)
        // const priceId = user?.priceId || me?.meta?.priceId || "";
        // let plan = "Basic";
        // if (priceId.toLowerCase().includes("pro")) plan = "Pro";
        // setPlanLabel(plan);
        //
        // if (plan === "Pro") setAmount(19900);
        // else setAmount(10900);
        ...
        // 🔹 NEW: derive current amount & plan from invoices
        // Prefer upcoming invoice (next cycle) if present, else latest paid
        ...
        if (ref && typeof ref.amount === "number") {
          setAmount(ref.amount); // Stripe amount in fils/cents
          // Simple mapping: >= 199 → Pro, else Basic
          const plan = ref.amount >= 19900 ? "Pro" : "Basic";
          setPlanLabel(plan);
        } else {
          // Fallback if somehow no invoices yet
          setAmount(null);
          setPlanLabel("");
        }
      } catch (e) {
                ...
                <div className="row g-3 mb-3">
                  ...
                  <div className="col-lg-6">
                    <div className="card h-100 bg-dark text-white border-0 rounded-4 p-4">
                      ...
                      <div className="display-5 fw-bold mb-2">
                        {amount == null ? "—" : `AED ${(amount / 100).toFixed(2)}`}
                        <span className="fs-6 text-secondary ms-1">/month</span>
                      </div>
