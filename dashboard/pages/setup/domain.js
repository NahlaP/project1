...
export default function DomainSetupPage() {
  ...
  const searchTlds = useMemo(() => {
    const base = Array.from(
      new Set([normalizeTld(tld), ...SIMPLE_TLDS])
    ).filter(Boolean);
    return base;
  }, [tld]);
  ...
  const goToCheckout = (opts = {}) => {
    ...
    let domainToSend = "";
    let domainType = (typeOverride || mode || "new").toLowerCase();
    ...
    if (includeDomain && domainToSend) {
      params.set("domain", domainToSend);
      // ✅ ALWAYS send domainType so backend doesn’t default to "new"
      params.set("domainType", domainType);
    }
  ...
  const renderNewDomainStatus = () => {
    ...
    if (availableCount > 0) {
      return (
        <div className="alert alert-success mt-3 py-2 px-3">
          We found <strong>{availableCount}</strong> available options. Select
          one to continue.
        </div>
      );
    }
      ...
      <div className="mt-3">
        <div className="border rounded-3 p-3 bg-white">
          ...
          <div className="mt-3">
            ...
            <div className="text-muted small mt-2">
              Prices are fetched in real time from Openprovider here. Checkout
              will re-check price on server.
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="row g-4">
      <div className="col-lg-7">
        <div className="mb-3">
          <h5 className="mb-1">Register a new domain</h5>
          <p className="mb-0">
            Search for a new domain. We’ll check availability and show pricing
            via <strong>Openprovider</strong>.
          </p>
        </div>
  ...
  const handleTransferSubmit = async (e) => {
    ...
    try {
      ...
      setTransferSuccess(
        res?.message || res?.msg || "Transfer request submitted."
      );

      // 2) get real transfer price from backend (same source checkout uses)
      const qRes = await api.get(
        `/api/openprovider/domains/transfer-price?domain=${encodeURIComponent(
          d
        )}`
      );
