...
export default function DomainSetupPage() {
  ...
  const [transferError, setTransferError] = useState("");
  const [transferSuccess, setTransferSuccess] = useState("");
  const [transferPrice, setTransferPrice] = useState(null); // transfer price in AED (from quote API)
  ...
  // ⬇️ can optionally include domain + prices in query string
  const goToCheckout = (opts = {}) => {
    const { includeDomain = false, transfer = null } = opts;
    ...
    // 🔁 TRANSFER MODE: pass domain + transfer price to checkout
    if (
      transfer &&
      transfer.domain &&
      typeof transfer.priceAed === "number" &&
      !Number.isNaN(transfer.priceAed)
    ) {
      const domain = transfer.domain.trim().toLowerCase();
      const priceAed = transfer.priceAed;
      const priceCents = Math.round(priceAed * 100);

      params.set("domain", domain);
      params.set("domainTransferPriceAed", String(priceAed));
      // we treat the full transfer price as extra to pay today
      params.set("domainExtraCents", String(priceCents));
    }
    // 🆕 NEW DOMAIN MODE (register)
    else if (
      includeDomain &&
      quote &&
      checkResult?.status === "available" &&
  ...
  const handleTransferSubmit = async (e) => {
    ...
    try {
      ...
      // 2) get transfer quote (price in cents) from backend
      try {
        const quoteRes = await api.get(
          `/api/resellerclub/domain/transfer-quote?domain=${encodeURIComponent(
            d
          )}`
        );

        const data = quoteRes?.data || quoteRes;
        const cents = data?.transferPriceCents;

        if (typeof cents === "number" && !Number.isNaN(cents)) {
          const aed = cents / 100;
          setTransferPrice(aed); // AED value – used for display and checkout
        } else {
          setTransferPrice(null);
        }
      } catch (innerErr) {
          ...
          <div className="d-flex gap-3">
            ...
            <button
              ...
              onClick={() => {
                ...
                if (d && transferPrice != null) {
                  // pass transfer domain + price into checkout URL
                  goToCheckout({
                    transfer: {
                      domain: d,
                      priceAed: transferPrice,
                    },
                  });
                } else {
                  // fallback: just go without domain info
                  goToCheckout();
                }
              }}
            >
