...
// server-side random if needed (invite mode)
const rand = (n=20) => {
  const alphabet = "ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789!@#$%^&*()-_=+";
  let p = ""; for (let i=0;i<n;i++) p += alphabet[Math.floor(Math.random()*alphabet.length)];
  return p;
};
...
export default async function handler(req, res) {
  try {
    // --------------------- CREATE ---------------------
    if (req.method === "POST") {
      // mode: 'password' (default) or 'invite'
      const { domain, user, password, quotaMB, sendWelcome, mode, alternateEmail } = req.body || {};
      if (!domain || !user) {
        return res.status(400).json({ error: "domain and user are required" });
      }

      // INVITE MODE (cPanel User Manager: send login link to alternate email)
      if (mode === "invite") {
        if (!alternateEmail) {
          return res.status(400).json({ error: "alternateEmail is required in invite mode" });
        }
        const q = toNum(quotaMB, 0);
        const params = {
          domain,
          username: user,
          password: password || rand(), // required by API; user can change after invite
          alternate_email: alternateEmail,
          send_invite: 1,
          // enable Email service for the subaccount + set quota
          "services.email.enabled": 1,
          "services.email.quota": q === 0 ? "unlimited" : String(Math.max(0, q)),
          "services.email.send_welcome_email": sendWelcome ? 1 : 0,
          type: "sub",
        };
        const r = await uget("UserManager/create_user", params);
        if (!okStatus(r.json)) {
          const details = firstError(r.json, r.text?.slice(0, 400));
          console.error("create_user failed", { params, details, raw: r.json || r.text });
          return res.status(502).json({ error: "create_user failed", details });
        }
        return res.status(200).json({ ok: true, created: `${user}@${domain}`, invited: alternateEmail });
      }

      // PASSWORD MODE (set password now)
      if (!password && !sendWelcome) {
        return res.status(400).json({ error: "password is required unless sendWelcome is true" });
      }
      const params = {
        domain,
        email: user,                           // local-part only
        password,
        quota: Math.max(0, toNum(quotaMB, 0)), // 0 = unlimited
    ...
    const dsum = async () => {
      ...
      return usedMainBytes;
    };

    const usedMainMB = mb(await dsum());
    rows.push({
