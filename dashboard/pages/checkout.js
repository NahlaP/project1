...
export default function CheckoutPage() {
  ...
  // explicit extra cents from query (mainly for transfer)
  const [domainExtraCentsParam, setDomainExtraCentsParam] = useState(0);

  // read domain info from query
  // new domain:   ?domain=vc.com&domainPriceAed=59.45&domainIncludedAed=50&domainExtraAed=9.45
  // transfer:     ?domain=vc.com&domainTransferPriceAed=59.45&domainExtraCents=5945
  useEffect(() => {
    if (!router.isReady) return;
    ...
    const priceAedFromNew =
      parseFloat(q.domainPriceAed?.toString() || "0") || 0;
    const includedAed =
      parseFloat(q.domainIncludedAed?.toString() || "0") || 0;
    let extraAedFromNew =
      parseFloat(q.domainExtraAed?.toString() || "0") || 0;

    const transferPriceAed =
      parseFloat(q.domainTransferPriceAed?.toString() || "0") || 0;

    const extraCentsParam =
      parseInt(q.domainExtraCents?.toString() || "0", 10) || 0;
    setDomainExtraCentsParam(extraCentsParam);

    // what we show as "Domain registration / year"
    const priceAed = priceAedFromNew || transferPriceAed;

    // effective extra (AED)
    let extraAed = extraAedFromNew;
    if (!extraAed && transferPriceAed && !includedAed) {
      // transfer case: full transfer price is treated as "extra"
      extraAed = transferPriceAed;
    }
  ...
  // if query gave exact cents (transfer), use that; otherwise derive from AED
  const domainExtraCents =
    domainExtraCentsParam ||
    Math.round((domainInfo.extraAed || 0) * 100);
