...
//       elements,
//       confirmParams: {
//         return_url: `${window.location.origin}/welcome?sid=${subscriptionId}`,
//       },
...
//   const billingCycle = (
//     router.query.billing?.toString() || "yearly"
//   ).toLowerCase();
...
//   // ➕ DOMAIN SUMMARY (from domain step)
...
//     const includedAed =
//       parseFloat(q.domainIncludedAed?.toString() || "0") || 0;
...
//         if (router.query.setup)
//           setup = toInt(router.query.setup.toString());
//         if (router.query.discount)
//           disc = toInt(router.query.discount.toString());

//         // default yearly offer
...
//   // convert domain AED → cents
//   const domainPriceCents = Math.round((domainInfo.priceAed || 0) * 100);
//   const domainIncludedCents = Math.round((domainInfo.includedAed || 0) * 100);
//   const domainExtraCents = Math.round((domainInfo.extraAed || 0) * 100);
...
//   // and (if domain in URL) wait until domainInfo is loaded
//   useEffect(() => {
//     if (
//       priceId &&
//       haveApiEmail &&
...
//       !clientSecret &&
//       !starting &&
//       (!hasDomainInUrl || domainInfo.name)
//     ) {
//       startCheckout();
//     }
//   }, [
...
//   // total: plan + setup + domain extra – discount
//   const totalTodayCents = Math.max(
//     0,
//     recurringCents - discountCents + setupFeeCents + domainExtraCents
//   );
...
//             <p>
//               Go back to <Link href="/choose-plan">Choose Plan</Link> and pick a
//               plan.
//             </p>
//           </div>
//         )}
...
//                 {billingCycle === "monthly"
//                   ? "Monthly Billing"
//                   : "Yearly Billing — 13 months for 12"}
//               </div>
//               {domainInfo.name && (
//                 <div className="domainTag">
//                   Domain: <span>{domainInfo.name}</span>
//                 </div>
//               )}
//             </div>
//             <div className="selRight">
//               <div className="price">
//                 {centsToMoney(recurringCents, currency)}
//               </div>
//               {discountCents > 0 && (
//                 <div className="save">
//                   Save {centsToMoney(discountCents, currency)}
//                 </div>
//               )}
//             </div>
//           </div>
...
//               {domainInfo.name && domainPriceCents > 0 && (
//                 <>
//                   <li className="domainRow">
//                     <span>
//                       Domain registration
//                       <div className="domainRowSub">{domainInfo.name}</div>
//                     </span>
//                     <strong>
//                       {centsToMoney(domainPriceCents, currency)} / year
//                     </strong>
//                   </li>

//                   {domainIncludedCents > 0 && (
//                     <li className="discount smallRow">
//                       <span>Included credit</span>
//                       <strong className="neg">
//                         -{centsToMoney(domainIncludedCents, currency)}
//                       </strong>
//                     </li>
//                   )}

//                   {domainExtraCents > 0 && (
//                     <li className="smallRow">
//                       <span>Extra to pay for domain</span>
//                       <strong>
//                         {centsToMoney(domainExtraCents, currency)}
//                       </strong>
//                     </li>
//                   )}
//                 </>
...
//                   <strong className="neg">
//                     -{centsToMoney(discountCents, currency)}
//                   </strong>
//                 </li>
//               )}
...
//                   {centsToMoney(totalTodayCents, currency)}
//                 </div>
//               </li>

//               <li className="badgeRow">
//                 <span className="badge green">1 Month Free</span>
//               </li>
...
//               By clicking “Start My Subscription”, you agree to our Terms of
...
//             <div className="card payment">Initializing checkout…</div>
//           )}
//         </section>
//       </main>
...
export default function CheckoutPage() {
  ...
  // domain only (NO PRICES FROM URL)
  const domain = router.query.domain?.toString() || "";
  const domainType = (router.query.domainType?.toString() || "new").toLowerCase(); // new|transfer|dns
  ...
  // plan summary from Stripe price
  const [planName, setPlanName] = useState("Selected Plan");
  const [currency, setCurrency] = useState("AED");
  const [recurringCents, setRecurringCents] = useState(0);
  const [discountCents, setDiscountCents] = useState(0);
  ...
  // Stripe invoice summary (SOURCE OF TRUTH)
  const [invoice, setInvoice] = useState(null);

  // Domain summary returned by backend (real provider price + AED charge)
  ...
  // ✅ load Stripe price meta for plan display (not total)
  useEffect(() => {
    let ignore = false;
    (async () => {
      if (!priceId) return;
  ...
  const startCheckout = useCallback(async () => {
    ...
    try {
      const payload = {
        ...
        // ✅ send only domain & type (no price)
        domain: domain || undefined,
        domainType: domainType || "new",
      };
      ...
      // ✅ backend returns Stripe invoice summary
      setInvoice(j.invoice || null);
      setDomainSummary(j.domain || null);
  ...
  }, [priceId, userEmail, billing, domain, domainType, router]);
  ...
  // ✅ Total Today MUST come from Stripe invoice (not from frontend math)
  const invoiceCurrency = (invoice?.currency || currency || "AED").toUpperCase();
  const totalTodayCents = invoice?.amount_due ?? null;
      ...
      <main className="shell">
        ...
        {!priceId && (
          <div className="card note">
            <p>No plan selected.</p>
            <p>
              Go back to <Link href="/choose-plan">Choose Plan</Link> and pick a plan.
            </p>
          </div>
        )}
        ...
        <section className="checkoutGrid">
          ...
          <div className="card selected" aria-label="Selected Plan">
            ...
            <div className="selRight">
              ...
              {discountCents > 0 && (
                <div className="save">Save {centsToMoney(discountCents, currency)}</div>
              )}
            </div>
          </div>
          ...
          <aside className="card summary" aria-label="Order Summary">
            ...
            <ul>
              ...
              {/* Domain line comes from backend (real provider price converted to AED cents) */}
              {domainSummary?.name && domainSummary?.chargeAedCents ? (
                <li className="domainRow">
                  <span>
                    Domain {domainSummary.type}
                    <div className="domainRowSub">{domainSummary.name}</div>
                    {Number.isFinite(domainSummary.priceUsd) && (
                      <div className="domainRowSub">
                        Provider: USD {domainSummary.priceUsd} (FX {domainSummary.fx})
                      </div>
                    )}
                  </span>
                  <strong>{centsToMoney(domainSummary.chargeAedCents, "AED")}</strong>
                </li>
              ) : null}
              ...
              <li className="totalRow">
                ...
                <div className="totalAmt">
                  {totalTodayCents == null
                    ? "—"
                    : centsToMoney(totalTodayCents, invoiceCurrency)}
                </div>
              </li>
