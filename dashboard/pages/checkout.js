...
export default function CheckoutPage() {
  ...
  const domain = router.query.domain?.toString() || "";
  const domainType = (router.query.domainType?.toString() || "new").toLowerCase(); // new|transfer|dns
  const authCode = router.query.authCode?.toString() || ""; // ✅ transfer only (optional)
  ...
  // Domain summary returned by backend
  const [domainSummary, setDomainSummary] = useState(null);
  ...
  const startCheckout = useCallback(async () => {
    ...
    // ✅ If transfer, require authCode (because webhook needs it)
    if (domain && domainType === "transfer" && !authCode.trim()) {
      setApiErr("Missing auth code for domain transfer. Go back and enter the EPP/Auth code.");
      return;
    }
    ...
    try {
      const payload = {
        ...
        // ✅ NEW: send authCode only for transfer
        authCode: domainType === "transfer" ? authCode.trim() : undefined,
      };
  ...
  }, [priceId, userEmail, billing, domain, domainType, authCode, router]);
