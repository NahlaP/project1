...
function AppointmentEditorPage() {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        const [app, team, testi, contact] = await Promise.all([
          fetch(`${backendBaseUrl}/api/appointment/${userId}/${templateId}`).then((r) => r.json()),
          fetch(`${backendBaseUrl}/api/team/${userId}/${templateId}`).then((r) => r.json()),
          fetch(`${backendBaseUrl}/api/testimonial/${userId}/${templateId}`).then((r) => r.json()),
          fetch(`${backendBaseUrl}/api/contact-info/${userId}/${templateId}`).then((r) => r.json()),
        ]);
        setAppointment(app);
        setTeam(team);
        ...
        console.error("❌ Failed to fetch appointment or related data", err);
      }
    };
    fetchAll();
  }, []);
