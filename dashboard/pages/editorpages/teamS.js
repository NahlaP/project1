...
/** cPanel upload endpoint (CORS already fixed on your server) */
...
function TeamEditor() {
  ...
  const reloadTeam = async () => {
    ...
    if (!res.ok) throw new Error('Load team failed');
    const json = await res.json();
    setMembers(normalizeMembers(json));
  };
  ...
  const getUploadToken = async (filename, type, size) => {
    ...
    // backend returns: { token, uploadUrl, expiresIn }
    return j;
  };

  /** Upload the file to cPanel using the token header */
  const uploadToCpanel = async (file, token) => {
    ...
    // returns { message: 'ok', url: 'https://sogimchurch.com/assets/img/xxxxx.jpg' }
    return j.url;
  };
