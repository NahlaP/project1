...
/* ---------- declare fields this section supports per template ---------- */
const TEMPLATE_PROFILES = {
  "sir-template-1": {
    hero: {
...
function HeroEditorPage() {
  ...
  const onResetDefault = async () => {
    ...
    setError("");
    try {
      const r = await fetch(endpoints.RESET, { method: "POST" });
      if (!r.ok) throw new Error(await readErr(r));
      await loadHero();
      setShowToast(true);
    } catch (e) {
        ...
        <div className="row g-5">
          ...
          <div className="col-lg-6">
            ...
            <div className="d-flex justify-content-between">
              <Button variant="outline-secondary" onClick={onResetDefault} disabled={saving || !templateId}>
