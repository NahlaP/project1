...
/**
 * We always call RELATIVE '/api/...': same origin => no CORS.
 * Upload goes to: /api/upload/:userId/:templateId/hero
 */
...
function HeroEditorPage() {
  ...
  const handleUploadImage = async (e) => {
    ...
    try {
      ...
      // ⛔️ DO NOT refresh from DB here; it would re-load the OLD key.
      // ✅ Show the newly uploaded image right away:
      const signed = await presign(data.key);
      setHero((p) => ({ ...p, imageKey: data.key, displayUrl: signed || p.displayUrl }));
      setSuccess("✅ Image uploaded! (Don’t forget to Save)");
    } catch (e2) {
      setError(String(e2.message || e2));
    } finally {
      setUploading(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Optional: refresh to confirm DB state
      await refreshHero();
      setSuccess("✅ Saved!");
    } catch (e2) {
      setError(String(e2.message || e2));
          ...
          <>
            <div className="row g-5 mb-4">
              <div className="col-lg-6">
                {hero.displayUrl ? (
                  <RBImage
                    ...
                    style={{ maxHeight: 350, objectFit: "cover", width: "100%" }}
                    onError={() =>
                      setError("Preview failed (URL may have expired). Click 'Refresh preview'.")
                    }
                  />
                ) : (
                  <div className="text-muted">No image uploaded yet</div>
              ...
              <div className="col-lg-6">
                ...
                <div className="small text-muted">
                  <div>
                    <strong>Stored key:</strong> {hero.imageKey || "(none)"}{" "}
                  </div>
                  <div>
                    <strong>Preview URL:</strong>{" "}
                    {hero.displayUrl ? "presigned (expires in a few min)" : "(none)"}{" "}
                  </div>
                </div>
              </div>
            </div>
