...
import BlogPreview from "../blogE";
import ContactPreview from "../contactE";
import FooterPreview from "../footerE";
...
export default function HomepagePreview() {
  const router = useRouter();
  const { id: rawId, templateId: fromQueryTpl, _r: refreshBump } = router.query;

  // 🔑 always the real, logged-in user (decoded from cookie/JWT by lib/api)
  const effectiveUserId = useMemo(() => getUserId(), []);
  ...
  // 1) Decide template, preferring ?templateId=, else backend selection
  useEffect(() => {
    let off = false;
    ...
    async function pickTemplate() {
      try {
        // A) URL wins
        if (typeof fromQueryTpl === "string" && fromQueryTpl.trim()) {
          if (!off) setEffectiveTemplateId(fromQueryTpl.trim());
          return;
        }
        // B) backend selected per-user template
        const sel = await api.selectedTemplateForUser(effectiveUserId);
        const backendT = sel?.data?.templateId || sel?.templateId;
        if (backendT && !off) {
          setEffectiveTemplateId(backendT);
          return;
        }
        // C) last fallback if nothing found — keeps editor usable
        if (!off) setEffectiveTemplateId("sir-template-1");
      } catch {
        if (!off) setEffectiveTemplateId("sir-template-1");
      }
    }

    if (effectiveUserId) pickTemplate();
    return () => {
      off = true;
    };
  }, [fromQueryTpl, effectiveUserId]);

  // 2) Resolve which page ID to open (Home page by slug/title)
  useEffect(() => {
    if (!router.isReady || !effectiveTemplateId || !effectiveUserId) return;
    ...
    const goTo = (id) => {
      ...
      const q = new URLSearchParams({
        ...
        // propagate cache-buster after reset (optional)
        ...(refreshBump ? { _r: String(refreshBump) } : {}),
      }).toString();
      ...
      try {
        ...
        const home =
          list.find(
            (r) =>
              r?.type === "page" &&
              ((String(r.slug || "").toLowerCase() === "home") ||
                (String(r.title || "").toLowerCase() === "home"))
          ) || list[0];
        goTo(home?._id);
      } catch (err) {
        console.error("Failed to resolve 'home' page:", err);
    ...
    setResolving(true);
    doResolve();
  }, [router.isReady, rawId, effectiveTemplateId, effectiveUserId, refreshBump]);
  ...
  // 4) Load sections for this page + template + user
  const loadSections = async (pid, tid) => {
    try {
      const res = await SectionsApi.list(effectiveUserId, tid);
      const assigned = (res.data || []).filter(
  ...
  useEffect(() => {
    if (!resolvedPageId || !effectiveTemplateId || !effectiveUserId) return;
    loadSections(resolvedPageId, effectiveTemplateId);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [resolvedPageId, effectiveTemplateId, effectiveUserId, refreshBump]);
                                  ...
                                  <Dropdown.Item
                                    onClick={() => {
                                      const sectionRoutes = {
                                        ...
                                        blog: "/editorpages/blogS",
                                        contact: "/editorpages/contactS",
                                        footer: "/editorpages/footerS",
                                        appointment: "/editorpages/appointmentS",
                                        testimonials: "/editorpages/testimonialS",
                                        team: "/editorpages/teamS",
                                      };
                                      const route =
                                        sectionRoutes[canon(section.type)] ||
                                        `/editorpages/section/${section._id}`;
                                      const q = new URLSearchParams({
                                        pageId: String(resolvedPageId),
                                        sectionId: String(section._id),
