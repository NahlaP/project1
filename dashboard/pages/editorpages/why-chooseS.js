...
function WhyChooseEditorPage() {
  const [state, setState] = useState({
    ...
    progressBars: [],
    bgOverlay: 0.5,
    bgImageUrl: "",   // public URL stored in DB (like hero/appointment)
  });
  ...
  // ------- cPanel upload (like Hero) -> returns PUBLIC URL -------
  ...
  const handleDeleteBg = async () => {
    setError("");
    try {
      // primary (hero/appointment-style)
      let res = await fetch(CLEAR_URL, { method: "POST" });
      if (!res.ok) {
        // fallback to older S3 DELETE /bg if your backend still has it
