...
//         }
//       } catch {}
//       // 3) Fallback (keep your old default)
...
//   // Load existing Appointment document for the resolved template
//   useEffect(() => {
...
//   // Choose file -> local preview ONLY (no upload here)
//   const onPickLocal = (e) => {
//     const file = e.target.files?.[0];
//     if (!file) return;
...
//     const form = new FormData();
//     form.append("image", draftFile);
//     // S3-backed route for appointment background (generic upload)
//     const res = await fetch(`${backendBaseUrl}/api/appointment/upload-bg`, {
//       method: "POST",
//       body: form,
//     });
...
//       // 3) Save the document (respect the resolved templateId)
//       const res = await fetch(
//         `${backendBaseUrl}/api/appointment/${encodeURIComponent(
//           userId
//         )}/${encodeURIComponent(templateId)}`,
