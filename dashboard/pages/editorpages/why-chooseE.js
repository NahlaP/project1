...
//  * 1) ?templateId=… → 2) backend-selected → 3) config fallback
//  */
...
//       } finally {
//         // 3) Fallback
//         if (!off) setTpl((v) => v || defaultTemplateId || "gym-template-1");
//       }
//     })();
//     return () => {
//       off = true;
...
// // Presign any S3 key via backend (supports both {url} and {signedUrl})
...
//     stats: [],
//     progressBars: [],
//     bgImageUrl: "", // may be full URL or a key
//     bgImageKey: "",
//     bgOverlay: 0.5,
//   });
...
//       // Otherwise presign a key (explicit key or non-absolute bgImageUrl)
//       const key =
//         data?.bgImageKey || (full && !isHttp(full) ? String(full) : "");
//       if (key) {
...
//       try {
//         const res = await fetch(
//           `${backendBaseUrl}/api/whychoose/${encodeURIComponent(
//             userId
...
//           progressBars: Array.isArray(data?.progressBars) ? data.progressBars : [],
//           bgImageUrl: data?.bgImageUrl || "",
//           bgImageKey: data?.bgImageKey || "",
//           bgOverlay: clamp(data?.bgOverlay ?? 0.5, 0, 1),
//         };
//         setDoc(normalized);
//         setDisplayUrl(await computeDisplayUrl(normalized));
//       } catch (e) {
//         console.error("❌ Failed to fetch Why Choose preview", e);
//         setDisplayUrl("");
//       }
//     })();
//   }, [templateId, userId, computeDisplayUrl]);

//   // compact preview slices
