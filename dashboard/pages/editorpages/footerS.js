...
// /** Normalize doc → always provide the right shapes & counts */
// const norm = (x) => {
//   const d = { ...DEFAULTS, ...(x || {}) };

//   // fill social to exactly 4
//   const srcSocial = Array.isArray(d.social) ? d.social : [];
//   d.social = Array.from({ length: 4 }, (_, i) => ({
//     label: srcSocial[i]?.label ?? DEFAULTS.social[i].label,
//     href: srcSocial[i]?.href ?? DEFAULTS.social[i].href,
//   }));

//   // fill links to exactly 3
//   const srcLinks = Array.isArray(d.links) ? d.links : [];
//   d.links = Array.from({ length: 3 }, (_, i) => ({
//     label: srcLinks[i]?.label ?? DEFAULTS.links[i].label,
//     href: srcLinks[i]?.href ?? DEFAULTS.links[i].href,
//   }));

//   // keep mailto/tel sane
//   d.emailHref =
//     d.emailHref ||
//     (d.emailLabel ? `mailto:${d.emailLabel}` : DEFAULTS.emailHref);
//   d.officePhoneHref =
//     d.officePhoneHref ||
//     (d.officePhone ? `tel:${String(d.officePhone).replace(/\s+/g, "")}` : DEFAULTS.officePhoneHref);
...
//       className="shadow-sm"
//       style={{
//         width: 896, borderRadius: 16, overflow: "hidden",
...
//           <div className="col-lg-6">
//             <div>
//               <div className="text-uppercase opacity-75 small">
//                 {data.topSubtitle || DEFAULTS.topSubtitle}
//               </div>
//               <h4 className="mt-1 underline">
//                 <a
//                   href={data.emailHref || `mailto:${data.emailLabel || "hello@Bayone.com"}`}
...
//                 __html: data.copyrightHtml || DEFAULTS.copyrightHtml,
//               }}
//             />
//           </div>
//         </div>
...
//     if (!templateId) return "";
//     return `${backendBaseUrl}/api/footer/${encodeURIComponent(userId)}/${encodeURIComponent(templateId)}`;
//   }, [userId, templateId]);
...
//               endpoint: <code>/api/footer/{defaultUserId}/{templateId}</code>
//             </div>
//           </div>
//         </Col>
//       </Row>
