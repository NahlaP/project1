...
// const CPANEL_UPLOAD_URL = 'https://sogimchurch.com/api/upload.php';
...
//         cache: 'no-store',
//       });
//       if (!res.ok) throw new Error('Load testimonials failed');
//       const data = await res.json();
...
//   /** Get short-lived upload token from backend */
...
//   /** Upload file to cPanel using token header */
//   const uploadToCpanel = async (file, token) => {
//     const fd = new FormData();
//     fd.append('file', file);
...
//     });
//     const j = await res.json();
//     if (!res.ok) throw new Error(j?.error || 'cPanel upload failed');
//     // returns { message: 'ok', url: 'https://sogimchurch.com/assets/img/xxxx.jpg' }
//     return j.url;
//   };
...
//       let imageUrl = '';
//       if (imageFile) {
//         const tok = await getUploadToken(imageFile.name, imageFile.type, imageFile.size);
//         imageUrl = await uploadToCpanel(imageFile, tok.token);
//       }
...
//       let imageUrl;
//       if (editingImageFile) {
//         const tok = await getUploadToken(
//           editingImageFile.name,
//           editingImageFile.type,
//           editingImageFile.size
//         );
//         imageUrl = await uploadToCpanel(editingImageFile, tok.token);
//       }
