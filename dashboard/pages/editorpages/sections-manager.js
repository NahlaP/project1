...
import { userId, templateId, backendBaseUrl } from '../../lib/config'; // ✅ imported
...
function SectionsManager() {
  ...
  const handleAddSection = async ({ type, title }) => {
    try {
      ...
      const { data: created } = await SectionsApi.create(userId, templateId, {
        type,
        title,
      ...
      if (type === 'page') {
        await axios.post(`${backendBaseUrl}/api/pages/generate`, {
          title: created.title,
