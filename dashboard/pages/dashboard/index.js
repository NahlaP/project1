...
const USER_ID_CONST = "demo-user";                // replace if you have auth
const FALLBACK_TEMPLATE_ID = "sir-template-1";    // safe default
const ALLOWED_TEMPLATES = ["sir-template-1", "gym-template"]; // ✅ use gym-template (matches S3)

// helper: normalize any legacy/mismatched IDs from API or DB
const normalizeTemplateId = (id) =>
  id === "gym-template-1" ? "gym-template" : id;
...
function TemplateChooserCard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // normalize any legacy ids in the list first
        const normalized = raw.map((t) => ({
          ...t,
          templateId: normalizeTemplateId(t.templateId),
        }));
        ...
        // choose selected from filtered list (fall back to first allowed)
        let activeTpl =
          normalizeTemplateId(sel?.data?.templateId ?? sel?.templateId) ??
          filtered?.[0]?.templateId ??
          null;
  ...
  async function choose(templateId) {
    try {
      ...
      setSaving(true);
      await api.selectTemplate(normId, userId);
      setSelected(normId);

      // pick a version tag for cookies (no UI change)
      const tplObj =
        templates.find((t) => t.templateId === normId) || null;
      const verTag = defaultVersionFor(tplObj);

      // keep cookie in sync (now writes version + user id as well)
      setTemplateCookie(normId, verTag, userId);

      // ping public host to sync
      fetch(
        `${PUBLIC_HOST}/?templateId=${encodeURIComponent(normId)}&v=${encodeURIComponent(
          verTag
        )}&r=${Date.now()}`,
        { mode: "no-cors", credentials: "include" }
      );
  ...
  function openReset(tpl) {
    ...
    setConfirmTpl({
      id: normalizeTemplateId(tpl.templateId),
      name: tpl.name || tpl.templateId,
      tag: defaultTag,
      versions,
    });
