...
// dashboard/pages/dashboard/index.js
// Works for Sir & Gym. On Select, also writes cookies on the cPanel domain via cookie-bridge.
...
function TemplateChooserCard() {
  ...
  // Write cookies on cPanel domain using a hidden iframe to cookie-bridge.html
  function writeBridgeCookiesOnPublicHost(templateId, verTag, uid) {
    return new Promise((resolve) => {
      const url =
        `${PUBLIC_HOST}/cookie-bridge.html` +
        `?templateId=${encodeURIComponent(templateId)}` +
        `&version=${encodeURIComponent(verTag)}` +
        `&userId=${encodeURIComponent(uid || "")}` +
        `&go=0`; // don't auto-redirect; we control navigation here
      ...
      // resolve on load or after 1.5s max
      ...
      // optional: listen for postMessage if you added it in the bridge
      const onMsg = (ev) => {
        if (ev?.data?.ok) {
          clearTimeout(timer);
          done();
  ...
  async function choose(templateId) {
    try {
      ...
      // 1) keep local cookies (for your app)
      setTemplateCookie(templateId, verTag, userId);

      // 2) write cookies on the PUBLIC_HOST (cpanel domain) via cookie-bridge
      await writeBridgeCookiesOnPublicHost(templateId, verTag, userId);

      // 3) optional: open the site with URL overrides for instant correctness
      window.open(
        `${PUBLIC_HOST}/?tpl=${encodeURIComponent(templateId)}&ver=${encodeURIComponent(verTag)}&r=${Date.now()}`,
        "_blank"
      );
    } catch (e) {
      alert(e.message || "Failed to select template");
  ...
  async function doReset() {
    ...
    try {
      ...
      if (selected === confirmTpl.id) {
        setTemplateCookie(confirmTpl.id, confirmTpl.tag, userId);
        await writeBridgeCookiesOnPublicHost(confirmTpl.id, confirmTpl.tag, userId);
      }
  ...
  // ✅ Open Editor for the currently selected template (works for Gym and Sir)
  async function openEditorForSelected() {
    try {
      const tplId = selected || FALLBACK_TEMPLATE_ID;
      const pageId = await api.getHomePageId(userId, tplId);
