...
/* -------- NEW: aggregate daily data into monthly buckets for the chart ----- */

function buildLastMonths(num = 6) {
  const out = [];
...
function aggregateVisitorsByMonth(days) {
  const byMonth = new Map();

  (Array.isArray(days) ? days : []).forEach((d) => {
    if (!d) return;
    const rawDate = d.date || d.day || d.createdAt;
    const count = Number(d.count ?? d.visits ?? 0) || 0;
    ...
    const existing = byMonth.get(key) || { key, label, count: 0 };
    existing.count += count;
    byMonth.set(key, existing);
  });

  if (byMonth.size === 0) {
    // no data at all → last 6 months with 0 visitors
    return buildLastMonths(6);
  }
...
export default function DashboardHome() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const info = await api.getDomainInfo();
        if (cancelled) return;
        setDomainInfo({
          loading: false,
          error: null,
        ...
        if (cancelled) return;
        console.error("[Dashboard] domain info error", err);
        setDomainInfo({
          loading: false,
          error: err?.message || "Failed to load domain info",
          data: null,
        });
      }
    })();
  ...
  // ✅ Fetch REAL visitor stats & aggregate by month
  useEffect(() => {
    let cancelled = false;
    ...
    (async () => {
      ...
      try {
        ...
        const months = aggregateVisitorsByMonth(days);

        const labels = months.map((m) => m.label); // Jan, Feb, ...
        const values = months.map((m) => m.count || 0);
  ...
  const domainUrl =
    domainInfo?.data?.siteUrl ||
    (domainInfo?.data?.domain
      ? `https://${domainInfo.data.domain}`
      : "");

  const openDomainSite = () => {
    if (!domainUrl) return;
  ...
  const domainBadgeText =
    domainInfo.data && typeof domainInfo.data.daysLeft === "number"
      ? `${domainInfo.data.daysLeft}d`
      : domainInfo.loading
      ? "…"
      : "—";

  const domainDisplayHost = domainUrl
    ? domainUrl.replace(/^https?:\/\//i, "")
    : "yourdomain.com";
                        ...
                        <Card.Body className="p-3">
                          ...
                          <div className="card_anim_body">
                            ...
                            <div className="col-info mt-2">
                              ...
                              <span>
                                {domainInfo.data?.domain || "mavsketch.com"}
                              </span>
                            </div>
                            <div className="col-info mt-1">
                              <span className="bold">Expiry date</span>
                              <span>
                                {formatDate(domainInfo.data?.expiresAt)}
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="button-wrapper d-flex flex-column gap-2">
                            <button
                              type="button"
                              className="primary-btn w-100"
                              onClick={openDomainSite}
                              disabled={!domainUrl || domainInfo.loading}
                            >
                              Visit your site
                            </button>
                          </div>
