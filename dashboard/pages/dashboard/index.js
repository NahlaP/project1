...
// // same url works fine for both templates
// // dashboard/pages/dashboard/index.js
// import { useEffect, useState } from "react";
// import { useRouter } from "next/router";
// import { Container, Row, Col, Card } from "react-bootstrap";
...
// dashboard/pages/dashboard/index.js
// Works on the same URL after refresh: reads templateId from cookie immediately,
// stays in sync across tabs, and updates server + cookie on selection.
...
function TemplateChooserCard({ onOpenEditor }) {
  ...
  // Show the correct selection immediately on refresh (before API resolves)
  useEffect(() => {
    const fromCookie = getTemplateCookie();
    if (fromCookie) setSelected(fromCookie);
  }, []);

  // Keep selection in sync across tabs/windows
  useEffect(() => {
    const off = onTemplateChange((tpl) => setSelected(tpl));
    return off;
  }, []);

  // Load templates + canonical selection from server
  useEffect(() => {
    let off = false;
    (async () => {
      try {
        ...
        // support both {data:{templateId}} and {templateId}
        const activeTpl = sel?.data?.templateId ?? sel?.templateId ?? null;
        if (activeTpl) setSelected(activeTpl); // server wins after load
      } catch (e) {
        if (!off) setError(e.message || "Failed to load templates");
      } finally {
        if (!off) setLoading(false);
  ...
  async function choose(templateId) {
    try {
      ...
      // 2) update cookie + broadcast to other tabs and this page
      setTemplateCookie(templateId);
      setSelected(templateId);

      // 3) (optional) ping public host to sync cookie there too (best-effort)
      if (typeof PUBLIC_HOST === "string" && PUBLIC_HOST) {
        fetch(`${PUBLIC_HOST}/?templateId=${encodeURIComponent(templateId)}&r=${Date.now()}`, {
          mode: "no-cors",
          credentials: "include",
        }).catch(() => {});
...
export default function DashboardHome() {
  ...
  // Resolve the Home page id using the template from cookie (so refresh works on same URL)
  useEffect(() => {
    let cancelled = false;
    (async () => {
      setFetchErr(null);
      try {
        const templateFromCookie = getTemplateCookie() || TEMPLATE_ID_CONST;
        const pageId = await api.getHomePageId(USER_ID_CONST, templateFromCookie);
        if (!cancelled) setHomePageId(pageId || null);
        if (!pageId) setFetchErr("Could not locate a 'home' page in API response.");
      } catch (err) {
        if (!cancelled) {
