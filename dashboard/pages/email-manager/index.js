...
export default function EmailManager() {
  ...
  const [creating, setCreating] = useState(false); // <-- separate flags
  const [deleting, setDeleting] = useState(false); // <-- separate flags
  const [error, setError] = useState(""); // page banner (load/delete)
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setDeleting(false); // <-- always reset
    }
  };
  ...
  const submitCreate = async () => {
    ...
    if (!domain || !user || (!password && !sendWelcome)) {
      pushToast("Domain, username and password are required (unless you only send a welcome email).", "error");
      return;
    }
    setCreating(true);
    try {
      ...
      if (stayAfterCreate) {
        setCreateForm(f => ({ ...f, user: "", password: "" }));
      } else {
        closeCreate();
      }
      pushToast(`Created ${user}@${domain}`, "success");
    } catch (e) {
      console.error(e);
      pushToast(e.message || "Create failed", "error"); // side toast only
    } finally {
      setCreating(false); // <-- always reset
    }
  };
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      ...
      >
        ...
        {/* page-wide banner for load/delete errors */}
        {error && (
            ...
            <div onClick={(e) => e.stopPropagation()} style={{ width: 560, background: "#fff", borderRadius: 8, padding: 16 }}>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 14 }}>
                ...
                <button
                  ...
                  disabled={creating || !createForm.domain || !createForm.user}
                >
