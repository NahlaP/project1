...
const CPANEL_USER = "mavsketc";
const CPANEL_WEBMAIL =
  process.env.NEXT_PUBLIC_CPANEL_WEBMAIL || "https://mavsketch.com:2096";
const CPANEL_UI =
  process.env.NEXT_PUBLIC_CPANEL_UI ||
  "https://mavsketch.com:2083/frontend/jupiter";
const DEFAULT_DOMAIN =
  process.env.NEXT_PUBLIC_DEFAULT_DOMAIN || "mavsketch.com";

const MAX_EMAIL_ACCOUNTS = 14;
// 🔒 fixed total email storage for this plan (5 GB)
const MAX_EMAIL_STORAGE_GB = 5;
...
/**
 * Convert strings like "1.31 MB", "0.20 GB", "500 KB" into GB (number).
 * Falls back to 0 if parsing fails.
 */
function parseHumanSizeToGB(str) {
  ...
  // Unknown unit – assume already GB
  return num;
}
...
export default function EmailManager() {
  ...
  const [createForm, setCreateForm] = useState(
    makeDefaultCreateForm(DEFAULT_DOMAIN)
  );
  ...
  // 🔔 show flash immediately if redirected with ?flash=...
  useEffect(() => {
    if (!router.isReady) return;

    let msg = router.query.flash ? String(router.query.flash) : null;
    let t = router.query.flashType
      ? String(router.query.flashType)
      : "success";
    ...
    if (!msg && typeof window !== "undefined") {
      try {
        const ssMsg = window.sessionStorage.getItem("EMAIL_FLASH_MSG");
        const ssType =
          window.sessionStorage.getItem("EMAIL_FLASH_TYPE") || "success";
        if (ssMsg) {
          msg = ssMsg;
          t = ssType;
          window.sessionStorage.removeItem("EMAIL_FLASH_MSG");
  ...
  const load = async () => {
    ...
    try {
      const r = await fetch(EMAILS_API);
      const d = await r.json();
      const safe = Array.isArray(d?.rows)
        ? d
        : { header: { available: "∞", used: 0 }, rows: [] };
      const rows = (safe.rows || []).map((x) => {
        const isSystem =
          Boolean(x.system) ||
          x.user === CPANEL_USER ||
  ...
  // 🔢 EMAIL STORAGE STATS (for "Email Capacity" widget)
  // usedGB is dynamic from cPanel rows; totalGB is fixed to MAX_EMAIL_STORAGE_GB (5 GB)
  const emailStorageStats = useMemo(() => {
    let totalUsedGB = 0;

    rows.forEach((r) => {
      totalUsedGB += parseHumanSizeToGB(r.usedHuman);
    });

    const used = totalUsedGB;
    const total = MAX_EMAIL_STORAGE_GB;
  ...
  useEffect(() => {
    setCreateForm((f) => ({
      ...f,
      domain: f.domain || domainOptions[0] || "",
    }));
  }, [domainOptions.length]);
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === "restricted")
      r = r.filter((x) => x.restrictions === "Restricted");
    if (filter === "exceeded") r = r.filter((x) => x.exceeded);
    if (filter === "system") r = r.filter((x) => x.system);
    return r;
  }, [rows, q, filter]);
  ...
  const visibleSelectable = useMemo(
    () => filtered.filter((r) => !r.system),
    [filtered]
  );
  const allVisibleSelected = useMemo(
    () =>
      visibleSelectable.length > 0 &&
      visibleSelectable.every((r) => selected[r.email]),
    [visibleSelectable, selected]
  );
  const someVisibleSelected = useMemo(
    () =>
  ...
  const headAllRef = useRef(null);
  useEffect(() => {
    if (miniAllRef.current)
      miniAllRef.current.indeterminate = someVisibleSelected;
    if (headAllRef.current)
      headAllRef.current.indeterminate = someVisibleSelected;
  }, [someVisibleSelected]);
  ...
  const submitCreate = async () => {
    ...
    if (!domain || !user) {
      pushToast("Domain and username are required.", "error");
      return;
    }
    if (mode === "invite" && !isEmail(alternateEmail)) {
      pushToast(
        "Enter a valid alternate email to send the login link.",
        "error"
      );
      return;
    }
    if (mode === "password" && !password && !sendWelcome) {
      pushToast(
        "Password is required unless you only send the welcome email.",
        "error"
      );
      return;
    }
  ...
  const canCreate =
    ...
    !!createForm.user &&
    (createForm.mode === "invite"
      ? isEmail(createForm.alternateEmail)
      : createForm.password || createForm.sendWelcome);
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", minHeight: "100vh" }}>
        ...
        >
          <Container fluid="xxl" className="dash-container">
            <h5 className="container-title">Email Accounts</h5>
            <p className="container-subtitle">
              Create, list, and manage email accounts.
            </p>
                  ...
                  <Col xs={6} md={7} lg={7} xl={7}>
                    <Card className="border-0 ion-card h-100 box-card">
                      <Card.Body className="position-relative px-4 pt-5 pb-4">
                        <div>
                          <div className="d-flex justify-content-between align-items-start mb-3">
                            <h5 className="card-title">Email Capacity</h5>
                            <div className="card-icon">
                              <img src="/icons/crown.svg" alt="Pro Plan" />
                            </div>
                          </div>
                        ...
                        <div className="card_wrapper_custom">
                          <h4 className="">
                            {emailStorageStats.usedGB.toFixed(1)}
                            <small className=""> out </small>
                            {emailStorageStats.totalGB > 0
                              ? emailStorageStats.totalGB.toFixed(1)
                              : "∞"}{" "}
                            <small className=""> /GB Used</small>
                          </h4>
                          ...
                          <div className="progress thin">
                            <div
                              ...
                              style={{
                                width: `${emailStorageStats.pctUsed.toFixed(
                                  1
                                )}%`,
                              }}
                            ></div>
                      ...
                      <div className="anim-card">
                        ...
                        >
                          <div>
                            ...
                            <p className="mb-0">
                              {currentCount} used of {MAX_EMAIL_ACCOUNTS}{" "}
                              accounts total
                            </p>
                          </div>
                          <div className="highlight-number">
                            <div className="number">
                              <span className="highlight">
                                {Math.max(
                                  0,
                                  MAX_EMAIL_ACCOUNTS - currentCount
                                )
                                  .toString()
                                  .padStart(2, "0")}
                              </span>
                              ...
                              <span className="sub-highlight">
                                {MAX_EMAIL_ACCOUNTS.toString().padStart(2, "0")}
                              </span>
                            </div>
                            <div className="number-info">Available</div>
                          </div>
              ...
              <Col xs={12} md={12} lg={12} xl={12}>
                <div className="anim-card-wrapper dark-bg cap-med">
                  <div className="anim-card">
                    ...
                    <Card.Body
                      className="p-3"
                      style={{ padding: "24px 20px 20px !important;" }}
                    >
                      <div>
                        <h6 className="card-title mb-1">Email List</h6>
                        <p className="mb-0" style={{ fontSize: "0.9rem" }}>
                          Overview of your email list
                        </p>
                      </div>
                        ...
                        <div className="table-wrapper">
                          <div className="widget-header">
                            <Row className="g-4 mb-5">
                              <Col xs={6} md={6} lg={6} xl={6}>
                                <div className="widget-wrapper">
                                  {["all", "restricted", "system", "exceeded"].map(
                                    (f) => (
                                      <div
                          ...
                          <div className="table">
                            <div className="table-row table-head">
                              ...
                              <div className="table-col">Restrictions</div>
                              <div className="table-col">
                                Storage: Used / Allocated / %
                              </div>
                              ...
                              {filtered.map((r) => {
                                ...
                                return (
                                  <div key={r.email} className="table-row">
                                    ...
                                    {/* Third col: Status */}
                                    <div className="table-col">
                                      {r.restrictions === "Restricted" ? (
                                        <div className="badge-soft-warning">
                                          <div className="dot"></div>
                                          Restricted
                                        </div>
                                      ) : (
                                        <div className="badge-soft-primary">
                                          <div className="dot"></div>
                                          Unrestricted
                                        </div>
                                      )}
                                    ...
                                    <div className="table-col">
                                      <div className="button-wrapper">
                                        ...
                                        <a
                                          href={`/email-manager/${encodeURIComponent(
                                            r.email
                                          )}`}
                                          className="link"
                                        >
                      ...
                      <label style={label}>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setCreateForm((f) => ({
                              ...f,
                              alternateEmail: e.target.value.trim(),
                            }))
                          }
                          placeholder="user@example.com"
                          style={input}
                    ...
                    >
                      <label style={label}>
                        ...
                        <input
                          ...
                          disabled={createForm.unlimited}
                          value={
                            createForm.unlimited ? 0 : createForm.quotaMB
                          }
                          onChange={(e) =>
                            setCreateForm((f) => ({
                              ...f,
                      ...
                      >
                        <input
                          ...
                          onChange={(e) =>
                            setCreateForm((f) => ({
                              ...f,
                              stayAfterCreate: e.target.checked,
                            }))
                          }
                        />
                        Stay on this dialog after Create (add multiple)
            ...
            {showQuota && (
              ...
              >
                <div
                  ...
                  role="dialog"
                  aria-modal="true"
                  aria-labelledby="quotaTitle"
                >
                  <h3 id="quotaTitle" style={{ margin: 0, marginBottom: 10 }}>
                  ...
                  >
                    ...
                    <strong>{MAX_EMAIL_ACCOUNTS}</strong> email accounts.
                    You currently have <strong>{currentCount}</strong>.
                    Please delete an existing account or increase your plan
                    limit before creating a new one.
                  </p>
                  <div
                    style={{
                      display: "flex",
  ...
  return (
    ...
    >
      ...
      <button
        onClick={onClose}
        title="Close"
        style={{ ...btn, margin: 0, padding: "2px 8px" }}
      >
        ×
      </button>
    </div>
