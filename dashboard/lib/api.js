...
// Use same cookie name as backend (ion7dev_auth) by default
const TOKEN_COOKIE =
  process.env.NEXT_PUBLIC_COOKIE_NAME ||
  process.env.COOKIE_NAME ||
  "ion7dev_auth";
...
/* ---------------- 401 guard: avoid infinite redirects ---------------- */
let handling401 = false;

/* ---------------- request helpers ---------------- */
async function request(path, init = {}) {
  // We no longer rely on Authorization header; backend uses httpOnly cookie
  const headers = Object.assign({}, init.headers || {});
  ...
  // ✅ If token missing/expired → redirect to signin ONCE
  if (res.status === 401) {
    if (typeof window !== "undefined") {
      const pathname = window.location.pathname || "";
      if (!handling401 && !pathname.startsWith("/authentication")) {
        handling401 = true;
        clearToken();
